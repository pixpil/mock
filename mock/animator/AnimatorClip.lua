...
function AnimatorKey:getKeyRange()
...
function AnimatorTrack:buildIdCurve( addEmptySpan )
	self:sortKeys()

	local keys = self.keys
	if not addEmptySpan then
		...
		for i, key in ipairs( keys ) do
			local t0 = key:getPos()
			local t1 = t0 + key:getLength() --epsilon
			if i > 1 and t0 > pt1 then --empty
				insert( ids, { pt1, 0 } )
			end
			insert( ids, { t0, i } )
			pt0 = t0
		...
		for i, k in ipairs( ids ) do
			local t, id = unpack( k )
			idCurve:setKey( i+1, t, id, MOAIEaseType.FLAT )
		end
		idCurve:setKey( count + 2, 1000000, 0, MOAIEaseType.FLAT )
		return idCurve
	end
...
function AnimatorTrack:onPreviewStart( state )
end

function AnimatorTrack:onPreviewStop( state )
end
