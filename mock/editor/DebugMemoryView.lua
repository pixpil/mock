...
function DebugMemoryView:getTitle()
	return 'Memory'
end
...
function DebugMemoryView:subOverview( gui, scn )
	...
	gui.Text( string.format( '_sys_vs:%dk',    memUsage._sys_vs or -1 ) )
	gui.Text( string.format( '_sys_rss:%dk',    memUsage._sys_rss or -1 ) )
end
...
function DebugMemoryView:subTrace( gui, scn )
	...
	if gui.Button( 'Report MOAI Tracking' ) then
		reportTrackingMOAIClasses()
	end

	if gui.Button( '!!!Dump Lua MemRef!!!!' ) then
		MOAILuaRuntime.setTrackingFlags( MOAILuaRuntime.TRACK_OBJECTS + MOAILuaRuntime.TRACK_OBJECTS_STACK_TRACE )
		-- MOAILuaRuntime.reportLeaks( 'MOAILeaks.log', nil, true )
		mri.m_cMethods.DumpMemorySnapshot("./", "1-Before", -1)
		-- memdump:initlize()
		-- memdump:dump( "memdump.log" )
		print( 'memeory dumped.' )
	end
...
function DebugMemoryView:subTextures( gui, scn )
	...
	local report = {}
	local count = 0
	for filename, tex in pairs( textures ) do
		count = count + 1
	...
	gui.Text( string.format( 'Total:%d', count ))
	if gui.Button( 'Print Report' ) then
		for i, entry in ipairs( report ) do
			print( i, entry[1], entry[2] )
		end
	end

	gui.BeginChild( 'textures' )
		gui.Columns( 2 )
