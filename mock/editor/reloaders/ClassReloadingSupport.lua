...
local function unloadRegistedObject( registry, m, info )
	local toRemove = {}
...
local function onModuleReleased( path, m )
	...
	local removedEntityClasses = 
		unloadRegistedObject( mock.getEntityRegistry(), m, 'entity' )

	local removedComClasses = 
	unloadRegistedObject( mock.getComponentRegistry(), m, 'component' )

	local removedGlobalClasses = 
	unloadRegistedObject( mock.getGlobalObjectClassRegistry(), m, 'global object' )
