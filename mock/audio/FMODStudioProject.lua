...
function FMODStudioProject:__init()
	...
	if osBrand == 'NS' then
		self.targetPlatform = 'Switch'
	elseif osBrand == 'iOS' or osBrand == 'Android' then
		self.targetPlatform = 'Mobile'
	else
...
function FMODStudioProject:updateChildAssetFromBanks( node )
	...
	for fmodPath, guid in pairs( self.strings ) do
		...
		if t == 'event' or t == 'snapshot' then
			local assetPath = basePath .. '/' .. t .. '/' .. subPath
...
function FMODStudioProject:loadAllBanks()
	...
	for i, f in ipairs( files ) do
		if f:endwith( '.bank' ) then
			...
			if f:endwith( 'Master Bank.bank' ) then
				blocking = true
			end
			if not self.banks[ f ] then
				self:loadBank( f, blocking )
			end
		end
	end
...
function FMODStudioProject:getIds()
	return self.ids
end

function FMODStudioProject:idToPath( id )
	return self.ids[ id ]
end

function FMODStudioProject:idToAsset( id )
	local fmodPath =self:idToPath( id )
	if not fmodPath then return false end
	local basePath = self.assetPath
	local t, subPath = fmodPath:match( '(%w+):/(.*)' )
	if t == 'event' then
		return basePath .. '/event/' .. subPath
	elseif t == 'snapshot' then
		return basePath .. '/snapshot/' .. subPath
	else
		return false
	end
end
...
function FMODStudioProject:getVCA( path )
