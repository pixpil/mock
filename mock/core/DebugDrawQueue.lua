...
dummpyDrawCommand = DebugDrawCommand ()
...
function DebugDrawQueueGroup:drawRect( x, y, x1, y1 )
	if DEBUGDRAW_ENABLED then
		return self:append( DebugDrawCommandRect( x,y,x1,y1 ) )
	else
		return dummpyDrawCommand
	end
end
...
function setDebugDrawEnabled( enabled )
	DEBUGDRAW_ENABLED = enabled ~= false
end