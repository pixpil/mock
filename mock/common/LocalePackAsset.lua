...
function LocalePack:affirmGUID()
	if not self.guid then
		self.guid = MOAIEnvironment.generateGUID()
	end
	return self.guid
end

function LocalePack:getGUID()
	return self.guid
end
...
function LocalePack:loadTranslationPack( locale )
	_stat( 'loading locale pack', locale )
	local translationPackPath = self.dataPath .. '/' ..locale
	local translationPack = false
	_log( 'loading locale pack', locale, translationPackPath )
	if MOAIFileSystem.checkFileExists( translationPackPath ) then
		_Stopwatch.start( 'locale_pack_loading' )
		-- local translationData = loadJSONFile( translationPackPath .. '.json' )
		-- translationPack = {}
		-- for k, v in pairs( translationData ) do
		-- 	translationPack[ k ] = makeI18nPack( locale, v )
		-- end
		...
		if m then
			...
			if ok then
				...
				if game:isEditorMode() then
					translationPack.__raw = translationData
				end
				for k, v in pairs( translationData ) do
		...
		_Stopwatch.stop( 'locale_pack_loading' )
		_log( _Stopwatch.report( 'locale_pack_loading' ) )

	else
		_warn( 'translation data not found', locale, translationPackPath )
	end
...
function LocalePack:load( configPath, dataPath )
	...
	if not MOAIFileSystem.checkFileExists( configPath ) then
		_error( 'locale pack not found', configPath )
		return false
	end
...
function LocalePack:loadConfig( configData )
	...
	self.guid = configData[ 'guid' ] or false
end
