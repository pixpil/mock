...
function ScriptedBehaviour:onSuspend( sstate )
	local onSuspend = self.delegate.onSuspend
	if onSuspend then
		onSuspend( sstate )
	end
	return ScriptedBehaviour.__super.onSuspend( self, sstate )
end

function ScriptedBehaviour:onResurrect( sstate )
	local onResurrect = self.delegate.onResurrect
	if onResurrect then
		onResurrect( sstate )
	end
	return ScriptedBehaviour.__super.onResurrect( self, sstate )
end
...
function ScriptedBehaviour:__serialize( objMap, reason )
	local dataInstance = self.dataInstance
	if not dataInstance then return end
	return _serializeObject( dataInstance, objMap, nil, nil, reason )
end
...
function ScriptedBehaviour:__deserialize( data, objMap, namespace )
	...
	local dataInstance = self.dataInstance
	if not dataInstance then return end
	return _deserializeObject( dataInstance, data, objMap, namespace, nil, 'fix_type' )
end
