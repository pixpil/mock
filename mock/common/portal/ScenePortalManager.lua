...
function ScenePortalManager:startPorting( portalId0, portalId1, options )
	...
	local sameScene = scenePath == toScenePath
	...
	emitGlobalSignal( 'scene_portal.pre_porting', portingSession )
...
function ScenePortalManager:finishPorting()
	...
	emitGlobalSignal( 'scene_portal.post_porting', portingSession )
end


function getScenePortalNamespace( scene )
	local portalManager = scene:getManager( 'ScenePortalManager' )
	if not portalManager then return false end
	return portalManager.portalNameSpace
end
