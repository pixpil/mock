...
function Stat:getValueCopy()
	return table.simplecopy( self.values )
end

function Stat:update( data, notify )
	if data then
...
function Stat:set( n, v, notify )
	local values = self.values
	local v0 = values[n]
	if v0 == v and notify ~= 'force' then return end
	values[n]	= v
...
function Stat:clearChangeListeners()
	self.changeListeners = {}
	self.globalChangeListenerList = {}
	self.changeListeners[ '*' ] = self.globalChangeListenerList
end

function Stat:clearChangeSignals()
	self.globalChangeSignal = false
	self.changeSignals = {}
end
...
function Stat:serialize()
	return valueTableCopy( self.values )
end

function Stat:deserialize( values )
	self.values = valueTableCopy( values )
end
