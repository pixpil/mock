...
local function stopwatchReportLog( ... )
	return _log( stopwatchReport( ... ))
end

local function stopwatchStopAndReportLog( ... )
	stopwatchStop( ... )
	return stopwatchReportLog( ... )
end

local function stopwatchProfileFunc( id, func, ... )
	stopwatchStart( id )
	func( ... )
	stopwatchStopAndReportLog( id )
end
...
_Stopwatch = {
	...
	stop                 = stopwatchStop,
	stopAndLog           = stopwatchStopAndReportLog,
	reset                = stopwatchReset,
	resetAll             = stopwatchResetAll,
	report               = stopwatchReport,
	logReport            = stopwatchReportLog,
	log                  = stopwatchReportLog,
	add                  = stopwatchAdd,
	profileFunc          = stopwatchProfileFunc
}
--------------------------------------------------------------------
local ProFi
...
function stopProfiler( outputFile )
	if ProFi then
		ProFi:stop()
		if outputFile then
			ProFi:writeReport( outputFile or 'profiler.log' )
		end
	end
end
