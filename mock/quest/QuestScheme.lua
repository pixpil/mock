...
function QuestNodeBase:__tostring()
	return string.format( '%s%s', self:__repr(), self.fullname or '???' )
end
...
function QuestNodeBase:isChildOf( node )
	local p = self.parent
	while p do
...
function QuestNodeBase:isParentOf( node )
...
function QuestNodeBase:pause( questState, from )
end

function QuestNodeBase:resume( questState, from )
end
...
function QuestConnection:sendSignal( questState, cmd, from )
	...
	elseif cmd == 'pause' then
		self.to:pause( questState, self )
	elseif cmd == 'resume' then
		self.to:resume( questState, self )
	end
end
...
function QuestNode:resume( questState, from )
	questState:setNodePaused( self, false )
end

function QuestNode:pause( questState, from )
	questState:setNodePaused( self, true )
end
...
--------------------------------------------------------------------
CLASS: QuestNodePause ( QuestNodeCommand )
	:MODEL{}

function QuestNodePause:getType()
	return 'pause'
end

function QuestNodePause:start( questState, from )
	for i, conn in ipairs( self.connectionsOut ) do
		if conn:evaluate( questState ) then
			conn:sendSignal( questState, 'pause', self )
		end
	end
end
...
--------------------------------------------------------------------
CLASS: QuestNodeResume ( QuestNodeCommand )
	:MODEL{}

function QuestNodeResume:getType()
	return 'resume'
end

function QuestNodeResume:start( questState, from )
	for i, conn in ipairs( self.connectionsOut ) do
		if conn:evaluate( questState ) then
			conn:sendSignal( questState, 'resume', self )
		end
	end
end
...
function QuestScheme:_loadNode( parentNode, data )
	...
	elseif tt == 'reset' then
		node = QuestNodeReset()
	elseif tt == 'pause' then
		node = QuestNodePause()
	elseif tt == 'resume' then
		node = QuestNodeResume()
	else
		node = QuestNode()
	end
	node.id          = data[ 'id' ]
