...
local function AuroraSpriteLoader( node )
	...
	deck:reserveSprites( data.partCount ) --one pair per frame component
	deck:reserveSpriteLists( #data.frames ) --one list per frame
	...
	for i, frame in ipairs( data.frames ) do
		...
		for j, part in ipairs( frame.parts ) do
			...
			deck:setSprite(partId, uvId, partId)
			partId = partId + 1
		end
		deck:setSpriteList( i, basePartId, partId - basePartId )
	end
