...
function TileMapGrid:coordToLoc( x, y )
	return self.grid:getTileLoc( x, y )
end
...
function TileMapLayer:__collect_dependency( collected, collector )
	if self.tilesetPath then collector( self, false, self.tilesetPath, collected ) end
	if self.materialPath then collector( self, false, self.materialPath, collected ) end
end
...
function TileMapLayer:worldToCoord( x, y )
	local x, y = self:worldToModel( x, y )
	return self:locToCoord( x, y )
end
...
function TileMapLayer:updateMaterial()
	...
	local instance = materialObject:affirmInstance( self )
	self:applyMaterial( instance )
end

function TileMapLayer:applyMaterial( materialInstance )
end
...
function TileMap:__collect_dependency( collected, collector )
	for i, layer in ipairs( self.layers ) do
		layer:__collect_dependency( collected, collector )
	end
end
...
function TileMap:onAttach( ent )
	for i, layer in ipairs( self.layers ) do
		...
		layer:applyMaterial( self:getMaterialInstance() )
	end
...
function TileMap:findLayerByTileset( path )
	for i, layer in ipairs( self.layers ) do
		if layer:getTilesetPath() == path then return layer end
	end
	return nil
end
...
function TileMap:applyMaterial( materialInstance )
	for i, layer in ipairs( self.layers ) do
		-- layer:applyMaterial( materialInstance )
