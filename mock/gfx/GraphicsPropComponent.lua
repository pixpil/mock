...
local propUsingRenderState = false
local _DefaultMaterialBatch = MOAIMaterialBatch.new()
setPropBlend( _DefaultMaterialBatch, 'normal' )

function setPropUsingRenderState( using )
	propUsingRenderState = using
end

function isPropUsingRenderState()
	return propUsingRenderState
end
...
local getRenderGroup = MOAINodeMgr.getRenderGroup
function GraphicsPropComponent:__init()
	self.billboard = false
	self.depthMask = false
	self.depthTest = false
	self.prop = markRenderNode( self:_createMoaiProp() )
end

function GraphicsPropComponent:applyDefaultMaterialBatch()
	self.prop:setParentMaterialBatch( _DefaultMaterialBatch )
end
...
function GraphicsPropComponent:getWorldSize()
	return self:getSize()
end

function GraphicsPropComponent:setWorldSize( w, h, d )
	return self:setSize( w, h, d )
end
...
function GraphicsPropComponent:applyMaterial( materialInstance )
	local prop = self.prop
	materialInstance:applyToMoaiProp( prop )
	if propUsingRenderState then
