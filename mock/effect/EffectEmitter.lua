...
function EffectEmitter:__init()
	...
	-- self.prop       = markRenderNode( MOAIGraphicsProp.new() )
	self.activeStates    = {}
	self.mirrorX    = false
	self.mirrorY    = false
...
function EffectEmitter:start()
	if self.defaultState then return self.defaultState end
...
function EffectEmitter:startIn( parentAction )
	if self.defaultState then return self.defaultState end
	local effect = self.effect or self.effectConfig
	if not effect then
		return
...
function EffectEmitter:getDefaultState()
	return self.defaultState
end

function EffectEmitter:getActiveStates()
