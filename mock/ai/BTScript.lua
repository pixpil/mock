...
function ParseContextProto:addLineChild()
	local node = { type = false, children = {}, indent = self.currentIndent, line = self.currentLineNo }
...
function ParseContextProto:isCurrentNodeRoot()
	return self.currentParent == self.rootNode
end
...
function ParseContextProto:parse_sub_def ( content, pos )
	local pattern = '^sub%s+([%w_]+)%s*'
	local s, e, match = string.find( content, pattern, pos )
	if not s then	return pos end
	if not self:isCurrentNodeRoot() then
		self:setErrorInfo( 
			string.format( 'sub tree should be defined at ROOT pos: %s', match )
		)
		return pos
	end
	local name = match
	if self:set( 'sub_def', name ) then
		self.subTreeNodes[ name ] = self.currentNode
		return e + 1
	else
		return pos
	end
...
function ParseContextProto:parse_sub_ref ( content, pos )
	local pattern = '^%.%s*([%w_]+)%s*'
	local s, e, match = string.find( content, pattern, pos )
	if not s then	return pos end
	local name = match
	if self:set( 'sub_ref', name ) then
		self.subTreeRefs[ self.currentNode ] = name
		return e + 1
	else
		return pos
	end
...
function ParseContextProto:parseLine( lineNo, l )
	...
	while true do
		...
		local pos0 = pos
		pos = self:parse_sub_def( l, pos )
		pos = self:parse_sub_ref( l, pos )
...
function ParseContextProto:parseSource( src )
	...
	--process subtree/reference
	for node, refName in pairs( self.subTreeRefs ) do
		local subTree = self.subTreeNodes[ refName ]
		if not subTree then
			local info = string.format( 'subtree %q not found @line: %d',  refName, lineNo )
			print( info )
			error( 'fail parsing' )
		end
		--make subnode copy and insert
		node.reference = subTree
	end
