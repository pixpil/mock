...
function BTAction:getArgS( k, default )
	return self:getArg( k, default )
end
...
function BTContext:setCondition( name, v )
	local v0 = self._conditions[ name ]
	if v0 == v then return false end
...
local function loadNode( data )	
	...
	if nodeType == 'commented' then 
		return false --ignore
	elseif nodeType == 'sub_def' then
		return false --ignore
	elseif nodeType == 'sub_ref' then
		local referredTree = data.reference
		local subTreeChildren = referredTree.children
		if not subTreeChildren then return false end
		for i, childData in ipairs( subTreeChildren ) do
			local node = loadNode( childData )
...
function BTController:pause()
	self.paused = true
end

function BTController:resume()
	self.paused = false
end

function BTController:start( initialStarting )
	if initialStarting and self.context._stopped then
		--already stopped
		return false
	end
	self:scheduleUpdate()
